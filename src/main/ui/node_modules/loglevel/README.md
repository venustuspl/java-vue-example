...
loglevel is written as a UMD module, with a single object exported. ES6 module
loaders & transpilers don't all handle this the same way. Some will treat the
object as the default export, while others use it as the root exported object.
Here are the two cases:

For tools like TypeScript and Browserify, which treat root exports as the root
value of the module:
...
For tools like Babel, and
Node.js's [experimental ESM support](https://nodejs.org/api/esm.html), which
treat root exports as the default export of the module:
...
* A `log.setDefaultLevel(level)` method.
  ...
  using `setDefaultLevel()`, it will still say as `trace` on subsequent page
...
* A `log.getLogger(loggerName)` method.
  ...
  loggers must have a name (which is a non-empty string). Calling `getLogger()`
  multiple times with the same name will return an identical logger object.
  ...
  level will not affect other loggers that have already been created.
...
Standard Streams - https://github.com/NatLibFi/loglevel-std-streams - Route
logging through STDERR in Node for easier log management.

Message Prefix - https://github.com/NatLibFi/loglevel-message-prefix - Dynamic (
timestamp/level) and static ('foo') message prefixing.

Message Buffer - https://github.com/NatLibFi/loglevel-message-buffer - Buffer
messages, and flush them on-demand later.
