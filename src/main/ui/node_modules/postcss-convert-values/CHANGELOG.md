...
* Does not convert `height:0%` to `height:0` (and the same for `max-height`), as they produce different results.
...
* Fixes an issue where the module would convert values in gradient/url functions since 1.3.0.
